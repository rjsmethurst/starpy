...
col = N.zeros(13*len(gz2data)).reshape(len(gz2data), 13)
...
col[:,12] = gz2data.field('specobjid')
...
one = colours[colours[:,12]==1470500645147535360]

print len(one)
print one

user =str(raw_input('okay?...'))
...
#print len(blue_c)
#
#age_save = '/Users/becky/Projects/Green-Valley-Project/bayesian/find_t_tau_old/gv/age_gv.npy'
...
samples, ss = sample(4, nwalkers, nsteps, burnin, start, w, one[:,0], one[:,4], one[:, 1], one[:, 5], age, one[:,3], one[:,2])
...
#f = open('/Users/becky/Projects/Green-Valley-Project/bayesian/find_t_tau_old/log.txt', 'a')
...
fig_s = corner_plot(samples[:,0:2], labels = [r'$ t_{smooth}$', r'$ \tau_{smooth}$'], extents=[[N.min(s[:,0]), N.max(s[:,0])],[N.min(s[:,1]),N.max(s[:,1])]], bf=[tqs_mcmc, taus_mcmc])
fig_s.savefig('triangle_t_tau_smooth_gv_'+str(len(samples))+'_'+str(time.strftime('%H_%M_%d_%m_%y'))+'.pdf')
fig_d = corner_plot(samples[:,2:4], labels = [r'$t_{disc}$', r'$\tau_{disc}$'], extents=[[N.min(s[:,2]), N.max(s[:,2])],[N.min(s[:,3]),N.max(s[:,3])]], bf=[tqd_mcmc, taud_mcmc])
